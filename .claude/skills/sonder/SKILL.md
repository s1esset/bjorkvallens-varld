...
| `node scripts/_vassfalleprobe.mjs` · `_grodfastprobe.mjs` | Fastnar grodan? `_vassfalleprobe` STÄLLER grodan i vassen vid kanten vänd mot väggen i varje pass (samma läge i båda armarna) och räknar kom-loss/ätna/skott-i-vassen — kontrollarmen fastnade 2/6, rättelsen 6/6 loss · `_grodfastprobe` är det envisa barnet (alltid närmaste insekt) med ögonblicksbild vid 25 s utan att äta. Går att köra mot HEAD före bajsloopen |
