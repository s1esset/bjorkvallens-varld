...
Ägarens beställning: *"roliga spel idéer som vi kan lägga till, som vi ej har och helst har fysik,
slumpmässiga händelser / saker i sig, som inte är likadant varje gång man spelar, gärna med
ragdolls, softbodies"*. Prövad mot alla 86 spel och mot post 1 här (inga dubbletter).

| # | id | Band | Stl | Fysiken | Nytt motorbygge |
|---|---|---|---|---|---|
| 1 | `popcornkalaset` | småbarn | M | värme per korn → mjuk popp → stel kropp i skålen | — |
| 2 | `nallens-stuntshow` | småbarn | M | ragdoll-nalle ur slangbella genom ett slumpat barnrum | `lib/ragdoll.js` (ur grodan) |
| 3 | `blackfisken-otto` | småbarn | M | 8 rep-armar med sugkoppar, mjukt huvud, flytvolym | — |
| 4 | `vattenballongerna` | småbarn | M | mjuk ballong med tryck → SPH-stänk mot varma djur | — |
| 5 | `slajmlabbet` | småbarn | M | ingredienser = mjukkroppens parametrar; delas och slås ihop | `Mjukkropp.dela()` |
| 6 | `tvattstugan` | småbarn | L | tyg (verlet-rutnät som `MeshPlane`) i trumman och på linan | `tyg.js` |
| 7 | `sandslottet` | småbarn | L | fallande sand (cellulär automat), slumpade vågor | `sand.js` |
| 8 | `bobo-i-rymden` | **storbarn** | L | ragdoll-Bobo mellan gravitationsbrunnar, seedat solsystem | ragdoll + förhandsbana genom flera fält |
| 9 | `gelebilen` | **storbarn** | L | mjuk bil som blåses upp, slumpad bana (JellyCar-lik) | kollision mjukkropp ↔ bana |
| 10 | `brobyggarna` | **storbarn** | L | fackverk med brottgräns, blandad djurkö | fackverk |
