...
### B8. Kulan fastnade mellan två slumpade föremål **[Quick]** — ✅ LÖST I `flipperspel` 2026-08-11

**Avgränsad med flit.** Ägarrapport: i `flipperspel` hamnade en slumpad stolpe 70–90 px från
väggen, och kulan (56 px) åkte in i gapet och blev liggande. Fixen är `src/lib/utplacering.js`
(`slumpaUt` + `hinderUrFysik`) och sonden `_banprobe.mjs` vaktar den: **0 fickor på 1 500
slumpade banor**, mot 5 107 före.

⚠️ **Det här är INGEN app-bred regel, och det är ett ägarbeslut.** Första versionen av den här
raden skrev in ett P0-krav för alla 73 spel och en åtgärdslista på 14 av dem. Ägaren drog
tillbaka det 2026-08-11: *"regeln gällde endast för flipperspelet då kulan fastnade mellan
2 objekt."* P0-raden är borttagen ur CLAUDE.md.

**Felet kräver tre saker samtidigt**, och det är därför det är sällsynt: föremål som får sin
position av slumpen · ett annat föremål som måste TA SIG FRAM mellan dem · och att det kan bli
liggande där i stället för att åka vidare. Ett spel som slumpar ut dekor, fläckar, fallande
föremål eller tappbara mål har inte det problemet.

**Om samma symptom dyker upp i ett annat spel** finns verktyget färdigt. Utifrån kodläsningen är
`rulla-bollen-hem` den enda tydliga tvillingen (hinder slumpas från nivå 6 och en boll ska rulla
förbi dem; koden kollar bara avstånd till mål och start). Bygg inget där förrän någon SETT det
hända — det är precis den överdrift som drogs tillbaka här.
