...
**Klara: 25 / 25** 🎉 — Byggplan 2 KLAR (alla 25 byggda, testade, simplade, committade)
...
| 24 | `trollblandning` | Trollkarlens Blandning | ✅ | ✅ | ✅ | ✅ | ✅ | Dra 2 element-droppar→kittel→reagerar (receptbok, ordnings-oberoende), ny droppe+bokrad, kedjor; hint-eskalering+auto-kombo=no-fail; fixade saknad makeMascot-import; test upptäckte Ånga; 0 fel |
| 25 | `loopdjuren` | Loopdjuren | ✅ | ✅ | ✅ | ✅ | ✅ | Loopimals-stil: dra rörelse/ljud-block i djurens loop-banor, playhead sveper→dansar i takt; tempo/tysta/cykla; mjukt öppet "klart" (alla aktiva djur spelat)=complete() en gång; 0 fel |

## Byggplan-2-logg
- **#25 loopdjuren** ✅ (SISTA): Byggd (fräsch agent, Loopimals-stil sequencer, ticker-playhead, ingen matter.js) + simplify-agent (tog bort döda sfx-fält/level-param/emoji-color-stores) + headless-test 0 fel (drog block i slots). `_loop`: `_t += deltaMS`, beat=floor(_t/beatMs)%slots, playhead-x interpolerad 280→1150; beat-byte→`_perform` per ifylld aktiv rad. 5 block (hopp/snurr/tut/klapp/röst m. `audio.sample('djur_<id>')`+voice-fallback). Kontroller: dra block + tempo 🐢900/🐇640 + tap-djur=tysta + tap-slot=cykla. Mjukt öppet "klart" = första varv där varje aktivt djur spelat ≥1 block → `complete()` EN gång (`_celebrated`-vakt), loopen rullar vidare. Röstrader (batchas): voiceIntro / djur-cry-fallback "Mu! Muu!" m.fl. Committad. **→ BYGGPLAN 2 KLAR 25/25.**
- **#24 trollblandning** ✅: Byggd (fräsch agent, recept-tabell + DragController, ingen matter.js) + **min bugfix: saknad `import { makeMascot }`** (kraschade vid start — bygget fångar ej runtime ReferenceError; test+screenshot fångade det) + simplify-agent (tog bort döda view/ring-handtag) + re-test 0 fel (test drog Eld+Vatten→upptäckte Ånga, fyllde bokrad 1/4 + ny droppe). RECIPES Map (sorterad par-nyckel, ordnings-oberoende): eld+vatten=ånga, eld+jord=lava, lava+vatten=sten, eld+lera=kruka, sol+vatten=regnbåge … kedjereaktioner. Oändliga hyll-källor; `_buildPlan` beroende-ordnad → 6s hint-eskalering→3 hints→wizard auto-kombo=garanterat nytt recept. Bobo m. trollhatt (ej namngiven människa). Röstrader (batchas): voiceIntro / "{Namn}! Vad fint!" / "Hmm... prova en annan!" / "Prova Eld och Vatten!". Committad.
