...
//
// drawIcon(key, size) returnerar en Graphics centrerad i (0,0), skalad så att
...
export function drawIcon(key, size = 100) {
  ...
  if (tpl === 'animal') {
    ...
    if (shape === 'star' || shape === 'gem') {
      ...
      g.circle(0, 0, 24 * S).fill(col).stroke({ width: 4, color: dk })
    ...
    if (form === 'plane') {
      ...
      g.lineTo(-40 * S, 12 * S).closePath().fill(col).stroke({ width: 4, color: dk })
      ...
      g.roundRect(-5 * S, -6 * S, 10 * S, 48 * S, 4 * S).fill(col).stroke({ width: 3, color: dk })
      g.roundRect(-30 * S, -30 * S, 60 * S, 24 * S, 5 * S).fill(0x9aa4b0).stroke({ width: 4, color: 0x6f7883 })
      ...
      g.roundRect(-4 * S, -5 * S, 42 * S, 10 * S, 5 * S).fill(col)
      g.roundRect(24 * S, 3 * S, 7 * S, 14 * S, 3 * S).fill(col)
      g.roundRect(34 * S, 3 * S, 7 * S, 10 * S, 3 * S).fill(col)
    } else if (form === 'brush') {
      g.roundRect(-5 * S, -34 * S, 10 * S, 44 * S, 4 * S).fill(col).stroke({ width: 3, color: dk })
      g.roundRect(-9 * S, 6 * S, 18 * S, 12 * S, 3 * S).fill(0xc3ccd4)
      g.moveTo(-9 * S, 18 * S).lineTo(9 * S, 18 * S).lineTo(5 * S, 38 * S).lineTo(-5 * S, 38 * S).closePath().fill(0xa78bfa)
    } else if (form === 'spoon') {
      g.ellipse(0, -20 * S, 14 * S, 19 * S).fill(col).stroke({ width: 4, color: dk })
      g.roundRect(-5 * S, -4 * S, 10 * S, 40 * S, 5 * S).fill(col).stroke({ width: 3, color: dk })
    } else if (form === 'umbrella') {
      g.arc(0, 2 * S, 40 * S, Math.PI, 0).fill(col).stroke({ width: 4, color: dk })
      for (const dx of [-20, 0, 20]) g.moveTo(dx * S, 2 * S).lineTo(dx * S, -26 * S).stroke({ width: 3, color: dk, alpha: 0.5 })
      g.roundRect(-3 * S, 2 * S, 6 * S, 36 * S, 3 * S).fill(0x8a5a3b)
      g.arc(-9 * S, 38 * S, 9 * S, 0, Math.PI).stroke({ width: 5, color: 0x8a5a3b })
    } else if (form === 'gift') {
      g.roundRect(-30 * S, -12 * S, 60 * S, 44 * S, 5 * S).fill(col).stroke({ width: 4, color: dk })
      g.roundRect(-34 * S, -22 * S, 68 * S, 14 * S, 5 * S).fill(col).stroke({ width: 4, color: dk })
      ...
      g.moveTo(-18 * S, 2 * S).lineTo(18 * S, 2 * S).lineTo(0, 40 * S).closePath().fill(0xd9a56b).stroke({ width: 4, color: 0xa9714a })
      g.circle(-10 * S, -10 * S, 15 * S).fill(0xffb3d1)
      g.circle(10 * S, -10 * S, 15 * S).fill(0xfff0a8)
      g.circle(0, -26 * S, 15 * S).fill(0xc4b1ff).stroke({ width: 3, color: 0xa78bfa })
      ...
      g.circle(-20 * S, -22 * S, 10 * S).fill(col)
      g.circle(20 * S, -22 * S, 10 * S).fill(col)
    ...
    if (form === 'bee') {
      ...
      g.ellipse(22 * S, -18 * S, 16 * S, 11 * S).fill({ color: 0xffffff, alpha: 0.7 }).stroke({ width: 2, color: 0xc3ccd4 })
      g.ellipse(0, 4 * S, 26 * S, 22 * S).fill(col).stroke({ width: 4, color: dk })
      ...
      g.ellipse(0, 6 * S, 30 * S, 34 * S).fill(col).stroke({ width: 4, color: dk })
